...
local function UpdateFrame(frame, force)
    if not force and (not ACT_AccountDB or not ACT_AccountDB.nickname_integrations or not ACT_AccountDB.nickname_integrations.Blizzard) then return end

	if not frame or frame:IsForbidden() or frame == PlayerFrame then return end

	local unit = frame.displayedUnit or frame.unit
	if unit and unit:match("nameplate") then return end

	local nameFrame = frame.name
	if not unit or not nameFrame or not UnitIsPlayer(unit) then return end
...
local function UpdateRaidFrames(force)
	if not CompactRaidFrameContainer or not CompactRaidFrameContainer.frameUpdateList or not CompactRaidFrameContainer.frameUpdateList.normal then return end
...
local function Update()
    if not ACT_AccountDB or not ACT_AccountDB.nickname_integrations or not ACT_AccountDB.nickname_integrations.Blizzard then return end
	UpdatePartyFrames()
	UpdateRaidFrames()
	UpdateTargetFrame()
end
...
local function Init()
	...
	if PartyFrame then
		for i = 1, 4 do
			...
			if memberFrame and memberFrame.name then
				hooksecurefunc(memberFrame.name, "SetText", function() UpdateFrame(memberFrame) end)
	...
	if TargetFrame then
		hooksecurefunc(TargetFrame.name, "SetText", function() UpdateFrame(TargetFrame) end)
		hookedFrames[TargetFrame.name] = true
	end

	hooksecurefunc("DefaultCompactUnitFrameSetup", function(frame)
		if not hookedFrames[frame.name] then
			hooksecurefunc(frame.name, "SetText", function() UpdateFrame(frame) end)
